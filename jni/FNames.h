...
string GetFNameFromID(uint32 index) {
    if (isUE423) {
        ...
        if (isBGMIndia()) {
            kaddr rawGNames = getRealOffset(Offsets::GNames);
            kaddr gNamesSource = getPtr(rawGNames);
            TNameEntryArray = getPtr(gNamesSource + 0x88);

            if (!TNameEntryArray) {
                TNameEntryArray = getPtr(rawGNames + 0x88);
            }
            if (!TNameEntryArray) {
                TNameEntryArray = getPtr(rawGNames);
            }
            if (!TNameEntryArray) {
                return "None";
            }
        } else if (isPtrDec && !isPUBGCN) {
            if(isPGLite) {
